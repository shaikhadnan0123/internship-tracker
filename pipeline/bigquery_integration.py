...
def upload_to_bigquery(csv_path="data/scored_applications.csv"):
    """
    Uploads the scored applications from CSV to BigQuery.
    """
    ...
    try:
        ...
        print(f"Uploading data to BigQuery table: {FULL_TABLE_REF} ...")
        job = client.load_table_from_dataframe(df, FULL_TABLE_REF, job_config=job_config)
        job.result()  # Wait for the load job to complete
        print("[BQ Upload] Data successfully uploaded to BigQuery!")
        return True
...
def get_applications_from_bq():
    """Queries all scored applications from BigQuery."""
    client = get_bigquery_client()
    if not client:
        return None
    try:
        query = f"SELECT * FROM `{FULL_TABLE_REF}` ORDER BY priority_score DESC"
...
def get_top_n_from_bq(n=5):
    """Queries top N applications from BigQuery."""
    client = get_bigquery_client()
    if not client:
        return None
    try:
        query = f"SELECT * FROM `{FULL_TABLE_REF}` ORDER BY priority_score DESC LIMIT {int(n)}"
        query_job = client.query(query)
        df = query_job.to_dataframe()
        return df
    except Exception as e:
...
def get_stats_from_bq():
    """Queries summary statistics from BigQuery."""
    client = get_bigquery_client()
    if not client:
        return None
    try:
        # Subqueries or aggregation queries
        query = f"""
        SELECT 
        ...
        stats = {
            ...
            "interviewing_count": int(row.get("interviewing_count", 0)),
            "rejected_count": int(row.get("rejected_count", 0)),
            "avg_priority_score": float(row.get("avg_priority_score", 0.0)),
            "platform_breakdown": platform_df.set_index("platform")["count"].to_dict()
        }
        return stats
    except Exception as e:
